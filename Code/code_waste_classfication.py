...
model = Sequential([
    Input(shape=(224, 224, 3)),  # ✅ Replaces input_shape in Conv2D
...
accuracy = test_accuracy  # e.g., 0.8953
...
bars = plt.bar(model_names, [0], color='skyblue')
bars[0].set_height(accuracies[0])
plt.ylim(0.5, 1.0)
plt.ylabel('Test Accuracy')
plt.title('Model Performance')
plt.bar_label(bars, labels=[f'{accuracies[0]:.2f}'], label_type='edge')
plt.show()
...
# Example: If your dataset has 2 classes
...
# ✅ Save trained model as "tuned_custom_model" for comparison
tuned_custom_model = model
...
def build_mobilenet_model(hp):
    ...
    # Tune dense layer size and dropout
...
# ✅ Automatically store the best model with final name (no need to assign later)
tuned_mobilenetv2 = tuner.get_best_models(num_models=1)[0]
...
evaluate_model(model, "Custom CNN", test_generator)

evaluate_model(tuned_mobilenetv2, "Tuned MobileNetV2", test_generator)

evaluate_model(tuned_custom_model, "Tuned Custom CNN", test_generator)
...
# Models to compare
models = {
    "Tuned Custom CNN": tuned_custom_model,
    "Tuned MobileNetV2": tuned_mobilenetv2
}
